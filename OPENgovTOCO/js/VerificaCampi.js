...
function isDate(strData, sDataDA, sDataA)
{
	...
	if(!isUndefined(sDataDA) || !isUndefined(sDataA))
	{
		...
		if(!isUndefined(sDataA) && trim(sDataA.toString())!='')
		{
		...
		if(!isUndefined(sDataDA) && trim(sDataDA.toString())!='')
		{
...
function isNumber(obj, intInteri, intDecimali, intMinVal, intMaxVal)
{	
	var strInteri='';
	var strDecimali='';
	var vetNumber;
...
//alert("strField=" + strField + "\nintInteri=" + intInteri + "\nintDecimali=" + intDecimali + "\nintMinVal=" + intMinVal + "\nintMaxVal=" + intMaxVal);
	...
	// *** Verifica parametri ***
	if(isUndefined(strField))					return true;
	if(IsBlank(strField))							return true;
	...
	{
		alert('Formato numerico non corretto');
	...
	{ 
		...
		//se non sono richiesti decimali non deve essere presente la virgola
	...
	if(!isUndefined(intMaxVal) || !isUndefined(intMinVal))
	{
		...
		//Verifica limite superiore
		if(!isUndefined(intMaxVal) && trim(intMaxVal.toString())!='')
		{
...
//alert('intMaxVal=' + intMaxVal + '\nintNewNumber=' +intNewNumber);
		...
		//Verifica limite inferiore
		if(!isUndefined(intMinVal) && trim(intMinVal.toString())!='')
		{
