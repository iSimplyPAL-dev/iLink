<%@ Page Language="vb" AutoEventWireup="false" Codebehind="RicercaImmobile.aspx.vb" Inherits="OpenUtenze.RicercaImmobile"%>
...
<HTML>
	<HEAD>
		...
		<script type="text/javascript">
			function SearchImmobileAnater()
			{	
				document.getElementById('loadGrid').src = 'ResultRicImmobile.aspx?Provenienza=<%=Request.item("Provenienza")%>&TxtVia='+document.RicercaUIAnater.TxtVia.value+'&TxtCivico='+document.RicercaUIAnater.TxtCivico.value+'&TxtInterno='+document.RicercaUIAnater.TxtInterno.value+'&TxtFoglio='+document.RicercaUIAnater.TxtFoglio.value+'&TxtNumero='+document.RicercaUIAnater.TxtNumero.value+'&TxtSubalterno='+document.RicercaUIAnater.TxtSubalterno.value
			}
			
			function ApriStradario(){
			    var CodEnte = '<% = Session("codente") %>';
				var TipoStrada = '';
				var Strada = '';
			...
			function RibaltaStrada(oggettoStrada){
			    document.getElementById('TxtVia').value = (oggettoStrada.TipoStrada + ' ' + oggettoStrada.Strada).replace(/&#(\d+);/g, function (m, n) { return String.fromCharCode(n); });
				document.getElementById('txtCodVia').value = oggettoStrada.CodStrada;
			}
	...
	<body class="Sfondo" bottomMargin="0" leftMargin="0" MS_POSITIONING="GridLayout">
		<form id="Form1" runat="server" method="post">
			<table cellSpacing="0" cellPadding="0" border="0" width="100%" style="LEFT: 0px; POSITION: absolute; TOP: 0px">
				<tr>
					<td>
										...
										<table id="TblParametri" cellSpacing="1" cellPadding="1" width="100%" border="0">
											<tr>
												<td colSpan="3">
													...
													<asp:imagebutton id="LnkApriStradario" runat="server" ImageUrl="../../Images/Bottoni/Listasel.png" ToolTip="Apri Ricerca Stradario"
